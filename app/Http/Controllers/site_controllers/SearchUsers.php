...
class SearchUsers extends Controller
{
    ...
    $indus='';
         ...
         $page_title="CareerSpoons - Candidates";
    	return view('client_views.user_related_pages.all_users',['candidates'=>$candidates,'industry'=>$industry,'degree'=> $degree,'qual'=>$qual,'city'=>$city,'cit'=>$cit,'gender'=>$gender,'career'=>$career,'quali'=>$quali,'indus'=>$indus,'page_title'=>$page_title]);
    }
    ...
    $candidates = $obj->do_filter_search($cit,$gender,$career,$quali,$indus);
         $city=DB::table('Add_cities')->get();
         $industry=DB::table('Company_industries')->get();
         $degree=DB::table('Add_degreelevel')->get();
         $qual=DB::table('Add_qualification')->get();
         // echo "<pre>";
         // print_r($candidates);
         $page_title="CareerSpoons - Filter Candidates";
         return view('client_views.user_related_pages.all_users',['candidates'=>$candidates,'industry'=>$industry,'degree'=> $degree,'qual'=>$qual,'city'=>$city,'cit'=>$cit,'gender'=>$gender,'career'=>$career,'quali'=>$quali,'indus'=>$indus,'page_title'=>$page_title]);
    }
}
